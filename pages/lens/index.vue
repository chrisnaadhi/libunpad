...
<template>
  <!-- Hero Banner -->
  <section class="bg-gradient-to-br from-unpad to-kandaga pt-12 pb-20 relative z-10">
    <div class="max-w-7xl ma px-4 text-center">
      ...
      <h1 class="text-white text-3xl sm:text-4xl font-800 m-0 mb-2 leading-tight">
      ...
      <div class="max-w-lg ma relative">
        <div class="i-mdi-magnify absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
        <input v-model="searchQuery" type="search" placeholder="Cari informasi, layanan, panduan..."
  ...
  <div class="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-gray-100 shadow-sm -mt-8 rounded-t-2xl">
  ...
  <main class="max-w-7xl ma px-4 py-6">
    <div class="grid grid-cols-1 lg:grid-cols-[240px_1fr] xl:grid-cols-[240px_1fr_300px] gap-8 items-start">
      <!-- ── Left Filter Sidebar (Desktop) ── -->
      <aside class="hidden lg:block sticky top-24">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sticky top-24">
          <h2 class="text-base font-700 text-gray-800 m-0 pb-4 mb-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <!-- Audience Filter -->
          <div class="mb-6">
            <p class="text-xs font-700 text-gray-500 uppercase tracking-wider mb-3">
              Audiens
            </p>
          ...
          <div class="mb-6">
            ...
            <div class="flex flex-col gap-1 max-h-60 overflow-y-auto pr-1 scrollbar-hide">
              <button
                class="flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all-200 text-left"
      ...
      <section class="min-w-0">
        ...
        <!-- Post Grid -->
        <div v-if="filteredPosts.length" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <NuxtLink v-for="post in filteredPosts" :key="post.id" :to="`/lens/content/${post.slug}`"
            ...
            <div class="relative h-48 overflow-hidden rounded-t-2xl bg-gray-100">
              <NuxtImg v-if="post.thumbnail" :src="handleAssets(post.thumbnail)" :alt="post.title"
                ...
                width="600" height="337" format="webp" />
              <div v-else
            ...
            <div class="p-5 flex flex-col flex-1 bg-white rounded-b-2xl">
              ...
              <p v-if="post.excerpt" class="text-sm text-gray-500 leading-relaxed line-clamp-2 flex-1 mb-4">
                {{ post.excerpt }}
              </p>
        ...
        <div v-else
          ...
          <button class="bg-unpad text-white px-6 py-2 rounded-full text-sm font-600 flex items-center gap-1.5"
            @click="resetFilters">
      ...
      <aside class="hidden xl:block sticky top-24 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div class="p-2">
            <NuxtLink v-for="profile in profilesData" :key="profile.id" :to="`/lens/p/${profile.abbreviation}`"
              ...
              <div
                ...
                <NuxtImg v-if="profile.profile_photo" :src="handleAssets(profile.profile_photo)" :alt="profile.name"
                  class="w-full h-full object-cover" width="40" height="40" format="webp" loading="lazy" />
                <div v-else class="i-mdi-domain w-5 h-5 text-unpad"></div>
              </div>
              <div class="min-w-0 flex-1">
                <p class="text-xs font-700 text-gray-700 truncate group-hover:text-unpad transition-colors-200">
                  {{ profile.name }}
                </p>
                <p class="text-xs text-gray-400">@{{ profile.abbreviation }}</p>
              </div>
              <div class="i-mdi-chevron-right w-4 h-4 text-gray-300 group-hover:text-unpad transition-colors-200" />
            </NuxtLink>
          </div>
        </div>
      </aside>
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showMobileFilter" class="fixed inset-0 bg-black/50 z-40 lg:hidden" @click="showMobileFilter = false">
      </div>
    </Transition>
    ...
    <Transition name="slide-up">
      <div v-if="showMobileFilter"
        ...
        <div class="p-5 border-b border-gray-100 flex items-center justify-between">
          <div class="w-10 h-1 bg-gray-200 rounded-full absolute top-2 left-1/2 -translate-x-1/2"></div>
        ...
        <div class="p-5 border-t border-gray-100 flex gap-3">
          <button class="flex-1 py-3 rounded-xl bg-gray-100 text-gray-600 font-600 text-sm" @click="resetFilters">
            Reset
          </button>
          <button class="flex-[2] py-3 rounded-xl bg-unpad text-white font-700 text-sm shadow-md"
            @click="showMobileFilter = false">
            Terapkan Filter
          </button>
        </div>
...
/* Custom Card Styling */
.post-card {
  --at-apply: bg-white rounded-2xl border border-gray-100 overflow-hidden flex flex-col cursor-pointer transition-all-300;
