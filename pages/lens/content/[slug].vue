...
const { data: profilesRaw } = await useAsyncData(
  `lens-post-profile-${post.id}`,
  () =>
    post.profile
      ? getItems({
...
const profile = profilesRaw.value?.[0];

// Fetch related posts (same category, excluding current)
const { data: relatedPosts } = await useAsyncData(
  `lens-related-${post.id}`,
  () =>
    getItems({
      collection: "kandaga_lens",
      params: {
    ...
    <div class="max-w-4xl ma px-4 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-8 items-start">
        ...
        <article class="space-y-6">
          ...
          <div
            v-if="post.thumbnail"
            class="rounded-2xl overflow-hidden aspect-video w-full bg-gray-1 shadow-sm"
          >
            <NuxtImg
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-2 p-6">
            ...
            <div class="flex flex-wrap items-center gap-2 mb-4">
              <span
                class="inline-flex items-center gap-1.5 text-xs font-700 px-3 py-1 rounded-full border"
                :class="getCategoryBadge(post.category).cls"
              >
                <div
                  :class="[getCategoryBadge(post.category).icon, 'w-3.5 h-3.5']"
                />
            ...
            <h1
              class="text-xl sm:text-2xl font-800 text-gray-8 m-0 mb-3 leading-snug"
            >
              {{ post.title }}
            </h1>
          ...
          <div
            ...
            class="bg-white rounded-2xl shadow-sm border border-gray-2 p-5"
          >
          ...
          <div v-if="relatedPosts?.length" class="space-y-3">
            <div
              class="flex items-center gap-2 text-xs font-700 text-gray-5 uppercase tracking-wider"
            >
        ...
        <aside class="space-y-5 lg:sticky lg:top-20">
          ...
          >
            <div
              class="bg-gradient-to-br from-unpad to-kandaga px-5 pt-5 pb-10 text-center"
            >
              <div
                class="w-16 h-16 rounded-full overflow-hidden ma mb-3 ring-3 ring-white/50"
              >
                <NuxtImg
            ...
            <div
              class="-mt-8 mx-4 mb-4 bg-white rounded-xl shadow-sm border border-gray-1 p-4 space-y-3"
            >
              <p
                v-if="profile.deskripsi"
                class="text-xs text-gray-5 leading-relaxed"
              >
              ...
              <div
                ...
                class="flex items-start gap-2 text-xs text-gray-5"
              >
                <div
                  class="i-mdi-map-marker-outline w-4 h-4 text-unpad flex-shrink-0 mt-0.5"
                />
              ...
              <div
                ...
                class="flex items-center gap-2 text-xs text-gray-5"
              >
                <div
                  class="i-mdi-email-outline w-4 h-4 text-unpad flex-shrink-0"
                />
                <a
                  :href="`mailto:${profile.contact_email}`"
                  class="hover:text-unpad transition-colors-200 truncate"
                >
