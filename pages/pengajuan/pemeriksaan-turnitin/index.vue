...
const uploadSurat = (event) => {
  ...
  if (file.size > maxSize) {
    ...
    notification.value =
      "Ukuran file melebihi 100 MB. Silahkan pilih file yang lebih kecil!";
    event.target.value = "";
    fileSurat.value = null;
    setTimeout(() => {
    ...
    <div class="bg-gradient-to-br from-unpad to-kandaga">
      <div class="max-w-3xl ma px-4 py-8">
        <div class="flex items-center gap-3 mb-1">
          <div
            class="i-mdi-file-document-edit-outline w-7 h-7 text-white/80 shrink-0"
          />
        ...
        <p class="text-white/70 text-sm mt-2">
          Tersedia untuk Mahasiswa, Dosen, dan Tenaga Pendidik Universitas
          Padjadjaran.
        </p>
        <NuxtLink
          to="/pengajuan"
    ...
    <div class="max-w-3xl ma px-4 py-6 flex flex-col gap-4">
      ...
      <div class="section-card">
        ...
        <div class="p-5 sm:p-6 flex flex-col gap-3">
          <div class="info-box bg-orange-50 border-orange-2">
            <div
              class="i-mdi-file-word-outline w-4 h-4 shrink-0 text-orange-5 mt-0.5"
            />
            <div class="text-sm text-gray-7 flex flex-col gap-1">
              <p>
                Unggah berkas dalam format word processor (<span
                  class="font-semibold"
                  >DOCX, DOC, ODT, atau ODF</span
                >).
                <span class="text-red-6 font-semibold"
                  >Tidak boleh dalam bentuk PDF.</span
                >
              </p>
              <p>
                Untuk meminimalisir angka similiaritas, tidak perlu melampirkan
                kata pengantar, daftar isi, daftar gambar, daftar tabel, lembar
          ...
          >
            <div class="flex items-center gap-2">
              <div
                class="i-mdi-list-box-outline w-4 h-4 shrink-0 text-gray-5"
              />
              <p class="text-xs font-semibold text-gray-7">
                Panduan Pemeriksaan Turnitin:
              </p>
            </div>
            <ol
              class="text-xs text-gray-6 list-decimal pl-5 flex flex-col gap-1"
            >
              <li>Wajib melampirkan Cover dari karya tersebut</li>
              <li>
                Kata-kata yang diwarnai bersumber pada website yang ada pada
                bagian akhir report (sesuai warna dan nomornya)
              </li>
              <li>
                Kalimat yang diwarnai sebaiknya diganti dengan parafrase yang
                memiliki arti sama
              </li>
              <li>
              ...
              <li>
                Persentase sebaiknya di bawah 20%; bila lebih, perbaiki dan
                periksakan kembali
              </li>
              <li>
                Surat pernyataan, kata pengantar, dan daftar isi sebaiknya tidak
                disertakan
              </li>
            </ol>
          </div>
          ...
          <div class="info-box bg-gray-50 border-gray-2">
            <div
              class="i-mdi-phone-outline w-4 h-4 shrink-0 text-unpad mt-0.5"
            />
            <div class="text-sm flex flex-col gap-0.5">
              <p class="font-semibold text-gray-7">
      ...
      <div class="section-card">
        <div class="section-card-header">
          ...
          <p class="ml-auto text-xs text-gray-4">
        ...
        <form @submit.prevent="kirimPengajuan" class="p-5 sm:p-6">
          <fieldset :disabled="isSubmitting" class="form-fieldset">
            <div class="flex flex-col gap-4">
              <div class="input-group">
                <label for="npm"
                  >NIP / NPM <span class="text-red-6">*</span></label
                >
              ...
              <div class="input-group">
                <label for="email"
                  >Email <span class="text-red-6">*</span></label
                >
                <input
                  type="email"
                  id="email"
              ...
              <div class="input-group">
                ...
                >
                  <div
                    class="i-mdi-file-word-outline w-4 h-4 text-blue-5 shrink-0"
                  />
                  <span class="flex-1 truncate font-medium text-gray-7">{{
                    fileSurat.name
                  }}</span>
                  <span class="text-gray-4 whitespace-nowrap text-xs">{{
                    fileSize
                  }}</span>
                </div>
                <p class="text-xs text-gray-4 italic mt-1">
                  Format: .doc, .docx, .odf, atau .odt — Maks. 100 MB. Tidak
                  boleh PDF.
                </p>
              </div>
            </div>
          ...
          <div
            ...
            :class="
              colorNotif === 'text-red-7'
                ? 'bg-red-50 border-red-2'
                : colorNotif === 'text-green'
          ...
          >
            <span v-if="submitStep === 'uploading'"
              >⏳ Mengunggah berkas...</span
            >
            <span v-else-if="submitStep === 'saving'"
              >💾 Menyimpan pengajuan...</span
            >
            <span v-else-if="submitStep === 'success'"
              >✅ Berhasil diajukan!</span
            >
