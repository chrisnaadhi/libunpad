...
<template>
  <li class="cursor-pointer z-2" @click="menuState.clickDropdown(menuName)">
    <div class="menu">
      ...
      <div class="">
        <div class="i-mdi-menu-down bg-gray" v-if="menuState.viewedMenuName !== menuName"></div>
        <div class="i-mdi-menu-up bg-gray" v-else></div>
      </div>
    </div>
    <span class="transition-all-500 w-full" :class="menuState.viewedMenuName === menuName ? '' : 'hidden'"
      @mouseenter="menuState.viewState = true" @mouseleave="menuState.leaveDropdown">
