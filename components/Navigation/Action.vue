...
<template>
  <div class="action-group xl:mr-0">
    <button type="button" class="btn mode" @click.stop="swapToggle" aria-label="Jam Layanan">
      <div class="i-mdi-calendar-clock bg-dark" />
    </button>
    <div v-if="status === 'authenticated'" class="relative hidden xl:block">
      <img v-if="data?.user?.image" :src="data?.user?.image ?? '/images/no-image.jpg'"
        class="w-10 h-10 rounded-full cursor-pointer" alt="Foto User" referrerpolicy="no-referrer"
        @click="toggleProfile" />
      <div class="account-dropdown" v-show="viewDropdown" :tabindex="-1" @pointerleave="viewDropdown = false">
        <p>{{ data?.user?.name }}</p>
        <p class="text-xs">{{ data?.user?.email }}</p>
        <div class="grid grid-cols-2 mt-2">
          <NuxtLink to="/keanggotaan" class="btn py-0 bg-orange text-xs text-white w-full">Keanggotaan</NuxtLink>
          <button class="btn py-0 bg-red text-xs text-white w-full" @click="logout">
            Logout
          </button>
        </div>
      </div>
    ...
    <button class="btn hamburger" role="menu" aria-label="Menu Toggle" @click="menuState.changeMenuState">
