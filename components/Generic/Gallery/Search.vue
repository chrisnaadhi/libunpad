  ...
  <form class="w-full relative flex" @submit.prevent="doSearch" role="search">
    <div class="flex flex-col items-center justify-center mt-2 w-full">
      <div class="relative w-full flex items-center justify-center mb-3">
        ...
        <input type="search" class="input-form" name="gallery-search" id="gallery-search" :placeholder="val" v-model="searchValue" @keyup="fetchGalleryItems" />
      ...
      <div class="bg-white rounded-xl mt--3 w-full" v-show="galleryItems?.length !== 0 && showSearchResults">
        <div class="max-h-75 overflow-auto">
          <div v-for="galeri in galleryItems" class="px-3 py-2">
            <NuxtLink class="no-underline" :to="`/gallery/${galeri.id}`">
              <h1 class="text-gray-5 text-xl transition-all-500 hover:text-orange">{{ galeri.judul }}</h1>
            </NuxtLink>
            <p class="text-xs text-gray">{{ trimDescription(galeri.keterangan_koleksi, 150) }}
              <span>
