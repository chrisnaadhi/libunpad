        ...
        <div class="max-h-75 overflow-auto">
          <div v-for="museum in museumItems" class="px-3 py-2">
            <NuxtLink class="no-underline" :to="`/museum/${museum.id}`">
              <h1 class="text-gray-5 text-xl transition-all-500 hover:text-orange">{{ museum.judul }}</h1>
            </NuxtLink>
            <p class="text-xs text-gray">{{ trimDescription(museum.keterangan_koleksi, 150) }}
              <span>
                <NuxtLink :to="`/museum/${museum.id}`">Lihat Koleksi</NuxtLink>
              </span>
            </p>
          </div>
          <div class="bg-orange-1 text-center py-2 cursor-not-allowed" v-if="museumItems?.length === 0">
            <p>{{ $t("noSearchResults") }}</p>
          </div>
          <div class="bg-orange-1 text-center py-2 cursor-pointer rounded-b-xl" v-else>
...
<style scoped>
.input-form {
  --at-apply: min-w-full h-14 px-10 py-3 md:pr-23 border-3 border-orange rounded-lg z-2 focus:(shadow-md outline-none);
}
...
.content-result {
  --at-apply: bg-white text-gray-6 text-left border-1 border-orange-3 rounded-lg max-h-75 overflow-auto transition-all-500;
}
