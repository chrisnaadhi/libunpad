  ...
  <form class="w-full relative flex" @submit.prevent="doSearch" role="search">
    <div class="flex flex-col items-center justify-center mt-2 w-full">
      <div class="relative w-full flex items-center justify-center mb-3">
        ...
        <input type="search" class="input-form" name="library-search" id="library-search" :placeholder="val" v-model="searchValue" @keyup="fetchLibraryItems" />
      ...
      <div class="bg-white rounded-xl mt--3 w-full" v-show="libraryItems?.length !== 0 && showSearchResults">
        <div class="max-h-75 overflow-auto">
          <div v-for="library in libraryItems" class="px-3 py-2">
            <NuxtLink class="no-underline" :to="`https://kandaga.unpad.ac.id:8010/index.php?p=show_detail&id=${library.biblioId}`" target="_blank">
              <h1 class="text-gray-5 text-xl transition-all-500 hover:text-orange">{{ library.title }}</h1>
            </NuxtLink>
            <p class="text-xs text-gray">{{ library.author }}
              <span>
                <NuxtLink :to="`https://kandaga.unpad.ac.id:8010/index.php?p=show_detail&id=${library.biblioId}`" target="_blank">Lihat Koleksi</NuxtLink>
              </span>
            </p>
          </div>
          <div class="bg-orange-1 text-center py-2 cursor-not-allowed" v-if="libraryItems?.length === 0">
